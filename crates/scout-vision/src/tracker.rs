...
impl Tracker {
    ...
    pub fn update(&mut self, dets: &[Detection]) -> TrackOutput {
        ...
        // Predict step: advance tracks by velocity (don't decay confidence yet - do after matching)
        ...
        // Track which tracks got matched
        ...
        // Decay confidence only for unmatched tracks
        for (ti, t) in self.tracks.iter_mut().enumerate() {
            if !matched_tracks[ti] {
                t.miss += 1;
                t.conf *= 0.95; // Faster decay for unmatched tracks
            }
        }
